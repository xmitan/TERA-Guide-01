	...
	function sActionStage(event) {
		...
		if ([3026, 3126].includes(whichmode) && [1000, 1001, 1002].includes(event.templateId) && event.stage==0) {
			...
			if ([212, 215].includes(skillid)) { // 内火(火爪)
				SendMessage((myDeBuff?CK_TipMsg[(0+bossWord+myDeBuff)%2]:"X") + "|内火|" + ((bossWord%2)?"恐惧(相同)":"愤怒(不同)"));
			...
			if ([213, 214].includes(skillid)) { // 内冰(冰爪)
				SendMessage((myDeBuff?CK_TipMsg[(1+bossWord+myDeBuff)%2]:"X") + "|内冰|" + ((bossWord%2)?"恐惧(相同)":"愤怒(不同)"));
		...
		// 司令官
		if ([3102, 3202].includes(whichmode) && event.templateId==1000 && event.stage==0) {
			if (!(bossSkillID = new_BOSS.find(obj => obj.id==skillid))) return;
			SendMessage(bossSkillID.msg);
		}
